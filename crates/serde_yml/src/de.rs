...
impl<'a> Parser<'a> {
    ...
    fn skip_blanks_and_comments(&mut self) {
        loop {
            // Skip only blank lines (newlines and lines that are
            // entirely whitespace), but preserve leading indentation
            // on content lines so peek_line_indent works correctly.
            loop {
                let rest = self.rest();
                if rest.starts_with('\n') {
                    self.advance_by(1);
            ...
            // Skip comment lines (# at current indent)
            let rest = self.rest();
