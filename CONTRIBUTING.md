...
make vet       # cargo-vet provenance (regenerate exemptions after a dep change)
```
...
Scopes: `metadata`, `metatags`, `utils`, `error`, `deps`, `repo`,
`fuzz`, `bench`.

Examples:

```text
fix(utils): unescape_html decodes each entity once, never its own output
build(deps): quick-xml 0.42 and noyalib =0.0.37
test(metadata): cover non-string leaves in every flattener
docs(adr): add 0004 first-party exact pins
```
...
For changes that touch the flattened metadata shape, the public API
...
- A minimal front-matter block or HTML fragment that reproduces the
  problem.
- Expected behaviour vs. actual behaviour.
- Rust version (`rustc --version`).
- metadata-gen version (`grep '^version' Cargo.toml`).
